...
Player::PlayerControllerComponent::PlayerControllerComponent () : m_isJumping (false), m_wasSpaceReleased (true) {
    m_camera = RenderingMaster::getInstance()->getCamera();
}
...
void Player::PlayerControllerComponent::input(Input &inputManager) {
    ...
    if (inputManager.getKey(SDLK_w)) {
        force += glm::vec3 (forward.x, 0, forward.z);
    } else if (inputManager.getKey(SDLK_s)) {
        force -= glm::vec3 (forward.x, 0, forward.z);
    }

    if (inputManager.getKey(SDLK_d)) {
        force += glm::vec3 (right.x, 0, right.z);
    ...
    if (glm::length(force) != 0) {
        ...
        physicsComponent->getRigidBody()->applyCentralForce(btVector3(force.x, 0, force.z));
    }
}
